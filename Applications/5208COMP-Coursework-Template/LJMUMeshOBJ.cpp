...
LJMUMeshOBJ::LJMUMeshOBJ(const std::wstring& pfilename)
{
	this->filename = pfilename;
	this->parseMesh();
}

void	LJMUMeshOBJ::parseMesh()
{
...
Vector3f LJMUMeshOBJ::toVec3(const std::vector<std::string>& ptokens)
{
...
Vector2f LJMUMeshOBJ::toVec2(const std::vector<std::string>& ptokens)
{
	assert(ptokens.size() >= 3);
	return Vector2f(std::stof(ptokens[1]), 1.0f - std::stof(ptokens[2]));
}

std::array<int, 3>	LJMUMeshOBJ::toIndexTriple(const std::string pstr)
{
...
int	LJMUMeshOBJ::wrapOffset(int pindex, int psize)
{
	if (pindex < 0)
